...
Page({
  ...
  admin() {
    ...
    if (that.data.userIsAdmin != 2) {
      if(that.data.userId == that.data.messageDetail.mangoUser.userId)
        return;
      wx.showModal({
        title: '用户',
        content:"用户名:" + that.data.messageDetail.mangoUser.userNickname ,
        cancelText: "取消",
        confirmText: "聊天",
        success: function(e) {
          if (e.confirm) {
            wx.navigateTo({
              url: '/pages/user_chat/user_chat?OtherUserId='+that.data.messageDetail.mangoUser.userId,
            });
          } else {
    ...
    wx.showModal({
      ...
      success: function(e) {
        if (e.confirm) {
          ...
          wx.navigateTo({
            url: '/pages/userDetailPage/userDetailPage?userId=' + that.data.messageDetail.userId
          });
        }
      }
    })
