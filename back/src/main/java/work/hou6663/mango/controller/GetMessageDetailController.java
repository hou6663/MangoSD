...
public class GetMessageDetailController {
    ...
    //获取最热全部帖子
    @PostMapping("/getMessage/getAllHotMessageDetail/{pageNumber}")
    public List<MangoMessage> getHotMessageDetail(@PathVariable Integer pageNumber) {
        PageHelper.startPage(pageNumber, 8);

        PageInfo<MangoMessage> pageInfo = new PageInfo<MangoMessage>(mangoMessageDetailService.getAllHotMessage());
        ...
        List<MangoMessage> allMessage = pageInfo.getList();
        MangoMessage.sortMessage(allMessage);
        return getImage(allMessage, mangoUserService, mangoMessageImagesService);
    }

    //获取最热种类帖子
    @PostMapping("/getMessage/getAllHotMessageDetail/{categoryId}/{pageNumber}")
    public List<MangoMessage> getHotMessageByCategoryId(@PathVariable Integer categoryId,@PathVariable Integer pageNumber) {
        PageHelper.startPage(pageNumber, 8);
